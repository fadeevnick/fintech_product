...
- Backoffice sanctions hit list/detail/decision APIs: `GET /api/v1/backoffice/sanctions-hits`, `GET /api/v1/backoffice/sanctions-hits/{id}`, `POST /api/v1/backoffice/sanctions-hits/{id}/decision`.
- `CLEAR_FALSE_POSITIVE` decision requires compliance role (`compliance_officer` or `senior_compliance`) and rationale length >= 20 characters; `backoffice_operator` decision attempts are denied.
