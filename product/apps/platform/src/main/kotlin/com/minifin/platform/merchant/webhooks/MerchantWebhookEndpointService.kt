...
class MerchantWebhookEndpointService(
    ...
    fun create(employee: MerchantEmployeeRecord, request: WebhookEndpointRequest): WebhookEndpointDto {
        ...
        try {
            repository.insert(id, employee.merchantId, validated.url, validated.enabledEventsJson, validated.status, validated.description, employee.id)
    ...
    private fun WebhookEndpointRecord.toDto(): WebhookEndpointDto = WebhookEndpointDto(
        id = id.toString(),
        url = url,
