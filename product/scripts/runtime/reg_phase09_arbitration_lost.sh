...
lost_replay_body="/tmp/minifin-${tag}-lost-replay.json"
...
create_dispute() {
  ...
  test "$(pa_public_post_payment_intent "${api_key}" "pi-${local_tag}" '{"amount":"18.25","currency":"EUR","description":"chargeback arbitration lost test"}' "${local_intent}")" = "201"
  ...
  test "$(pa_curl -sS -b "${enduser_cookie}" -o "${out_dispute}" -w "%{http_code}" -X POST "${base_url}/api/v1/card-payments/${local_intent_id}/disputes" -H "Content-Type: application/json" -d '{"reasonCode":"goods_not_received","narrative":"Runtime CHB-05 dispute"}')" = "201"
...
submit_evidence() {
  ...
  local evidence_json="{\"narrative\":\"Representment package for arbitration LOST runtime path.\",\"attachments\":[{\"fileName\":\"receipt.pdf\",\"contentType\":\"application/pdf\",\"storageKey\":\"chargeback-evidence/runtime/receipt.pdf\",\"sizeBytes\":35,\"contentBase64\":\"${evidence_payload_base64}\"}]}"
...
pa_create_api_key "${PA_COOKIE_JAR}" "Phase09 arbitration lost" "${merchant_body}"
...
merchant_id="$(p05_platform_psql "select merchant_id from chargeback.disputes where id='${dispute_id}'::uuid;")"
submit_evidence "${dispute_id}" "${evidence_body}"

rationale="Network simulation found cardholder claim valid for runtime arbitration lost path."
test "$(pa_curl -sS -o "${lost_body}" -w "%{http_code}" -X POST "${base_url}/api/v1/backoffice/disputes/${dispute_id}/arbitration" -H "Authorization: Bearer ${operator_token}" -H "Content-Type: application/json" -d "{\"outcome\":\"LOST\",\"rationale\":\"${rationale}\"}")" = "200"
merchant_debit_journal_id="$(node -e "const j=JSON.parse(require('fs').readFileSync(process.argv[1],'utf8')); if(j.data?.state!=='LOST'||j.data?.outcome!=='LOST'||!j.data?.arbitrationJournalId) process.exit(1); console.log(j.data.arbitrationJournalId);" "${lost_body}")"
test "$(p05_platform_psql "select state from chargeback.disputes where id='${dispute_id}'::uuid;")" = "LOST"
test "$(p05_platform_psql "select count(*) from chargeback.disputes where id='${dispute_id}'::uuid and arbitration_outcome='LOST' and arbitration_journal_id='${merchant_debit_journal_id}'::uuid;")" = "1"
...
test "$(p05_platform_psql "select count(*) from audit.audit_log where event_type='chargeback.arbitration_lost' and subject_id='${dispute_id}'::uuid;")" = "1"
...
test "$(pa_curl -sS -o "${lost_replay_body}" -w "%{http_code}" -X POST "${base_url}/api/v1/backoffice/disputes/${dispute_id}/arbitration" -H "Authorization: Bearer ${operator_token}" -H "Content-Type: application/json" -d "{\"outcome\":\"LOST\",\"rationale\":\"${rationale}\"}")" = "409"
test "$(node -e "const j=JSON.parse(require('fs').readFileSync(process.argv[1],'utf8')); if(j.errors?.[0]?.code!=='invalid_state') process.exit(1);" "${lost_replay_body}"; echo ok)" = "ok"

printf 'CHB-05 arbitration LOST pass dispute_id=%s provisional_journal_id=%s merchant_debit_journal_id=%s user_id=%s\n' "${dispute_id}" "${provisional_journal_id}" "${merchant_debit_journal_id}" "${user_id}"
