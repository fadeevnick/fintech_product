...
- Phase 07 Slice 05 compliance read-audit in `platform`: compliance sanctions hit detail reads synchronously write `audit.read_audit_log` with actor, subject/resource, purpose, decision and role metadata; sanctions list reads and forbidden role attempts do not create detail ALLOW rows. `AUD-03` has runtime evidence.
...
- Phase 08 Slice 07 AML alert review decisions in `platform`: backoffice `GET /api/v1/backoffice/aml-alerts`, `GET /{id}`, and `POST /{id}/decision` allow operators and compliance officers to close, escalate, or mark AML alerts for SAR; `CLOSED_FALSE_POSITIVE` on an auto-frozen alert unfreezes the actor; `MARKED_FOR_SAR` is restricted to compliance roles; `aml.aml_alert_decisions` persists every decision; `aml.alert_reviewed` audit rows written. `AML-05` has runtime evidence.
...
Phase 06 Slice 04 adds the first captured-payment settlement path. Phase 06 Slice 05 adds deterministic local settlement fee splits. Phase 06 Slice 06 adds an Acquirer-local settlement projection:
...
- one balanced `CARD_PAYMENT_SETTLEMENT` ledger journal is posted per settlement item;
- deterministic local fee rates are applied: issuer interchange `1.20%`, network assessment `0.15%`, acquirer margin `0.65%`;
- settlement item stores gross, merchant net and fee component amounts;
- settlement journal credits merchant net plus `ISSUER_INTERCHANGE_REVENUE`, `NETWORK_ASSESSMENT_REVENUE` and `ACQUIRER_MARGIN_REVENUE`.
- `POST /internal/settlement/publish-projections?limit=100` publishes Platform settlement items into Acquirer;
- Acquirer exposes service-auth-protected `POST /internal/settlement/projections`;
- Acquirer persists one `merchant_settlement.balance_projection` row per Platform settlement item, including batch id, merchant id, payment intent id, gross, merchant net, fee components, currency and Platform settled timestamp;
- projection ingestion is idempotent through `platform_settlement_item_id`.
