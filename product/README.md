...
- Phase 06 Slice 06 acquirer settlement projection across `platform` and `acquirer`: Acquirer stores `merchant_settlement.balance_projection`, Platform publishes settled item projections through `POST /internal/settlement/publish-projections`, and Acquirer ingests them idempotently by Platform settlement item id. `SET-03` has a retained runtime script.
...
`SET-01`, `SET-02` and `SET-03` are implemented. Payouts, refunds, chargebacks, bank file export, scheduled reconciliation jobs and merchant dashboard settlement UI are not implemented in these slices.
